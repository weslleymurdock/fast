...
# Usando ENV em vez de ARG garante que não haverá prompts interativos nos estágios filhos
...
RUN cp -a /opt/dependencies/. /usr/local/ \
    && ldconfig \
    && git clone "${ASTERISK_G72X_REPOSITORY}" . \
    && git fetch --all --force \
