...
ARG OPUS_VERSION
ARG OPUS_REPOSITORY
RUN git clone --branch "${OPUS_VERSION}" "${OPUS_REPOSITORY}" /usr/src/opus \
    && cd /usr/src/opus \
...
RUN git clone --branch "${OPENH264_VERSION}" "${OPENH264_REPOSITORY}" /usr/src/openh264 \
    && cd /usr/src/openh264 \
    && git fetch --tags --force \
...
COPY --from=codec-bcg729 /opt/artifact/ /opt/bcg729/
COPY --from=codec-opus /opt/artifact/ /opt/opus/
COPY --from=codec-openh264 /opt/artifact/ /opt/openh264/
ENV PKG_CONFIG_PATH="/opt/openssl/lib/pkgconfig:/opt/bcg729/lib/pkgconfig:/opt/openh264/lib/pkgconfig:/opt/opus/lib/pkgconfig"
...
RUN cp -a /opt/dependencies/. /usr/local/ \
    && ldconfig \
    && git clone "${ASTERISK_G72X_REPOSITORY}" /usr/src/asterisk-g72x \
...
LABEL \
    ...
    org.opencontainers.image.licenses="MPL2.0" \
    io.fast.asterisk.version="${ASTERISK_VERSION}" \
    io.fast.pjsip.version="${PJSIP_VERSION}" \
...
COPY docker/entrypoint.sh \
    /usr/local/bin/fast-freepbx-entrypoint

COPY docker/healthcheck.sh \
    /usr/local/bin/fast-freepbx-healthcheck

COPY docker/asterisk-rtp.conf \
...
RUN ldconfig \
    ...
    && sed -i \
        's/^export APACHE_RUN_USER=.*/export APACHE_RUN_USER=asterisk/' \
        /etc/apache2/envvars \
    && sed -i \
        's/^export APACHE_RUN_GROUP=.*/export APACHE_RUN_GROUP=asterisk/' \
        /etc/apache2/envvars \
    && mkdir -p \
        /etc/asterisk \
        /var/lib/asterisk \
...
HEALTHCHECK \
    ...
    --start-period=120s \
    --retries=5 \
    CMD ["/usr/local/bin/fast-freepbx-healthcheck"]
